...
yves.inspector = function (options) {
    ...
    return function (obj, label, opts) {
        ...
        if (myopts.html && !myopts.stream) {
            return '<pre class="yves" style="padding:8px;background-color: black;color: white;overflow: auto;border-radius: 10px;'+((myopts && myopts.styles && myopts.styles.all)?('color:'+myopts.styles.all+';'):'')+'">'+result+'</pre>';
        } else {
            return result;
        }
    };
...
yves.console = function(namespace) {
  ...
  const debugLog = yves.debugger(namespace+'console:log')
  console.log = function() {return debugLog.apply(yves,argumentsToArray(arguments))}
  const debugInfo = yves.debugger(namespace+'console:info')
  console.info = function() {return debugInfo.apply(yves,arguments)}
  const debugWarn = yves.debugger(namespace+'console:warn')
  console.warn = function() {return debugWarn.apply(yves,arguments)}
  const debugError = yves.debugger(namespace+'console:error')
  console.error = function() {return debugError.apply(yves,arguments)}
  const debugDir = yves.debugger(namespace+'console:dir')
  console.dir = function() {
    // yves.inspector()(argumentsToArray(arguments))
    let args=argumentsToArray(arguments)
    args.unshift('%y')
    return debugDir.apply(yves,args)
  }
}
...
function yvesInclude(key,includes)
{
  if (includes && includes.length) {
    for (let i in includes) {
      ...
      if (typeof includes[i] == 'object' && includes[i] instanceof RegExp && includes[i].test(key)) return true;
    }
    return false
  }
  return true
...
function yvesExclude(key,excludes)
{
  if (excludes && excludes.length) {
    for (let i in excludes) {
      ...
      if (typeof excludes[i] == 'object' && excludes[i] instanceof RegExp && excludes[i].test(key)) return true;
    }
  }
  return false
}
...
function yvesObfuscate(key,obfuscates)
{
  if (obfuscates && obfuscates.length) {
    for (let i in obfuscates) {
      ...
      if (typeof obfuscates[i] == 'object' && obfuscates[i] instanceof RegExp && obfuscates[i].test(key)) return true;
    }
  }
  return false
}
